...
from biome_fm.plastic._fileops import add, move, remove
...
def test_add_calls_cm_add_with_paths(tmp_path):
    ...
    with patch("biome_fm.plastic._fileops.run_cm") as m:
        add([p], tmp_path)
    m.assert_called_once_with(["add", str(p)], cwd=tmp_path, timeout=None)
...
def test_remove_calls_cm_remove(tmp_path):
    ...
    with patch("biome_fm.plastic._fileops.run_cm") as m:
        remove(p, tmp_path)
    m.assert_called_once_with(["remove", str(p)], cwd=tmp_path, timeout=None)
...
def test_move_calls_cm_move_with_src_dst(tmp_path):
    ...
    m.assert_called_once_with(["move", str(src), str(dst)], cwd=tmp_path, timeout=None)
