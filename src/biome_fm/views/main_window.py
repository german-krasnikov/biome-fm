...
class MainWindow(QMainWindow):
    ...
    dup_tab_requested = Signal()
    save_session_requested = Signal()
    restore_session_requested = Signal()
    select_pattern_requested = Signal()
    select_criteria_requested = Signal()
    copy_path_requested = Signal()
    copy_names_requested = Signal()
    bulk_rename_requested = Signal()
    quick_cd_requested = Signal()
    jump_requested = Signal()
    bookmarks_requested = Signal()
    bookmark_toggle_requested = Signal()
    swap_requested = Signal()
    target_eq_source_requested = Signal()
    zoom_in_requested = Signal()
    zoom_out_requested = Signal()
    zoom_reset_requested = Signal()
    fullscreen_requested = Signal()
    treemap_requested = Signal()
    ...
    def _build_menubar(self) -> None:
        mb = self.menuBar()
        mb.setContextMenuPolicy(Qt.ContextMenuPolicy.PreventContextMenu)
        ...
        fm.addAction(a)
        fm.addSeparator()
        a = QAction("Duplicate Tab\tCtrl+Alt+T", self)
        a.triggered.connect(lambda _: self.dup_tab_requested.emit())
        fm.addAction(a)
        a = QAction("Save Session\tCtrl+Shift+S", self)
        a.triggered.connect(lambda _: self.save_session_requested.emit())
        fm.addAction(a)
        a = QAction("Restore Session\tCtrl+Shift+O", self)
        a.triggered.connect(lambda _: self.restore_session_requested.emit())
        fm.addAction(a)
        fm.addSeparator()
        a = QAction("&Quit", self)
        a.setMenuRole(QAction.MenuRole.NoRole)
        ...
        em.addAction(a)
        em.addSeparator()
        a = QAction("Select by Pattern\tCtrl+G", self)
        a.triggered.connect(lambda _: self.select_pattern_requested.emit())
        em.addAction(a)
        a = QAction("Select by Criteria\tCtrl+Shift+G", self)
        a.triggered.connect(lambda _: self.select_criteria_requested.emit())
        em.addAction(a)
        em.addSeparator()
        a = QAction("Copy Path\tCtrl+Shift+C", self)
        a.triggered.connect(lambda _: self.copy_path_requested.emit())
        em.addAction(a)
        a = QAction("Copy File Names\tAlt+Shift+N", self)
        a.triggered.connect(lambda _: self.copy_names_requested.emit())
        em.addAction(a)
        em.addSeparator()
        a = QAction("Bulk Rename Editor\tCtrl+Shift+R", self)
        a.triggered.connect(lambda _: self.bulk_rename_requested.emit())
        em.addAction(a)
        ...
        for label, sig in [
            ...
            nm.addAction(a)
        nm.addSeparator()
        a = QAction("Quick CD\tAlt+C", self)
        a.triggered.connect(lambda _: self.quick_cd_requested.emit())
        nm.addAction(a)
        a = QAction("Jump to Frecent\tCtrl+J", self)
        a.triggered.connect(lambda _: self.jump_requested.emit())
        nm.addAction(a)
        nm.addSeparator()
        a = QAction("Bookmarks...\tAlt+B", self)
        a.triggered.connect(lambda _: self.bookmarks_requested.emit())
        nm.addAction(a)
        a = QAction("Bookmark Toggle\tCtrl+D", self)
        a.triggered.connect(lambda _: self.bookmark_toggle_requested.emit())
        nm.addAction(a)
        nm.addSeparator()
        a = QAction("Swap Panes\tCtrl+U", self)
        a.triggered.connect(lambda _: self.swap_requested.emit())
        nm.addAction(a)
        a = QAction("Target = Source\tCtrl+Shift+U", self)
        a.triggered.connect(lambda _: self.target_eq_source_requested.emit())
        nm.addAction(a)
        ...
        vm.addAction(a)
        vm.addSeparator()
        a = QAction("Zoom In\tCtrl+=", self)
        a.triggered.connect(lambda _: self.zoom_in_requested.emit())
        vm.addAction(a)
        a = QAction("Zoom Out\tCtrl+-", self)
        a.triggered.connect(lambda _: self.zoom_out_requested.emit())
        vm.addAction(a)
        a = QAction("Reset Zoom\tCtrl+0", self)
        a.triggered.connect(lambda _: self.zoom_reset_requested.emit())
        vm.addAction(a)
        vm.addSeparator()
        a = QAction("Fullscreen\tF11", self)
        a.triggered.connect(lambda _: self.fullscreen_requested.emit())
        vm.addAction(a)
        vm.addSeparator()
        ws_sub = vm.addMenu("&Workspaces")
        self.workspace_menu = ws_sub
        ...
        tm.addAction(a)
        tm.addSeparator()
        a = QAction("Storage Treemap\tCtrl+Alt+M", self)
        a.triggered.connect(lambda _: self.treemap_requested.emit())
        tm.addAction(a)
        a = QAction("Large File Finder\tCtrl+Alt+G", self)
        a.triggered.connect(lambda _: self.large_files_requested.emit())
        tm.addAction(a)
        a = QAction("Task Runner\tCtrl+Shift+M", self)
        a.triggered.connect(lambda _: self.task_runner_requested.emit())
        tm.addAction(a)
        ...
        hm = mb.addMenu("&Help")
        a = QAction("Keyboard Shortcuts\tF1", self)
        a.triggered.connect(lambda _: self.shortcuts_help_requested.emit())
        hm.addAction(a)
        a = QAction("About", self)
        a.setMenuRole(QAction.MenuRole.NoRole)
        a.triggered.connect(lambda _: self.about_requested.emit())
        hm.addAction(a)
